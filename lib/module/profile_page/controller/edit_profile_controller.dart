...
class EditProfileController extends GetxController {
  ...
  Future<void> saveProfile(String email, String name, String phone, String address) async {
    try {
      ...
      _storage.remove('profileData'); // Hapus data lokal setelah berhasil di-upload
  ...
  Future<void> _uploadProfileIfOnline() async {
    ...
    if (storedProfile != null) {
      try {
        await _firestore.collection('users').doc(userId).set(storedProfile, SetOptions(merge: true));
        _storage.remove('profileData');
        print('Profile data uploaded successfully to Firestore and removed from GetStorage.');
      } catch (e) {
  ...
  void selectProfileImage() async {
    ...
    if (image != null) {
      ...
      final savedImage = await File(image.path).copy('${directory.path}/profile_$timestamp.jpg');
  ...
  void loadProfileImage() async {
    ...
    if (profileFile is File && profileFile.existsSync()) {
      profileImage.value = profileFile; // Update the profile image if a file exists
    }
  }
}
